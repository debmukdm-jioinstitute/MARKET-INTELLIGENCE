...
- **Currency-consistent discount rate**: USD uses the live 10Y Treasury; other currencies use a local-currency risk-free rate plus a **country risk premium** (`cost of equity = rf + β × (ERP + CRP)`). The rf and CRP tables are static, approximate Damodaran-style defaults (`src/lib/models/country.ts`) — surfaced in Assumptions and flagged by a currency-mismatch check.
...
**Projection** — revenue by total growth *or* volume × price; margins, D&A, capex, SBC, working-capital days roll into an integrated balance sheet / income statement / cash flow. Includes an **automatic revolver** (draws to hold minimum cash, repaid from surplus), interest on **average balances** (circularity solved by fixed-point iteration), **NOL carryforward** with a usage cap, an effective tax rate that **converges to the marginal rate**, and optional **EBIT-margin fade** to a long-run target. A 6-year lookback re-derives every average for cyclicals (mid-cycle margins).
