...
- **Beta**: regressed from monthly stock-vs-index returns, Blume-adjusted (`0.67×raw + 0.33×1`, switchable to raw via the `use_blume` input), falling back to 1.0 with under 24 months of data.
...
- **Projection**: revenue growth fades linearly from its historical CAGR and reaches the terminal growth rate in the final forecast year; margins, D&A, capex, stock-based compensation, and working-capital days (DSO/DIO/DPO) roll forward from historical averages into a fully integrated balance sheet, income statement, and cash flow statement.
...
- **Sign & tax rules**: no tax credit on projected losses, dividends only from positive net income, interest income only on positive cash, and the Gordon terminal value returns 0 (and the sensitivity cell blank) when WACC ≤ g rather than a negative value.
