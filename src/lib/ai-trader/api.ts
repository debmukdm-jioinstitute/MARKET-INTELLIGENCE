/** Browser calls the Next.js proxy; server-side jobs may call Flask directly. */
const UPSTREAM = (process.env.AI_TRADER_API_URL || process.env.NEXT_PUBLIC_AI_TRADER_API_URL || "http://127.0.0.1:5050").replace(/\/$/, "");

export const API_BASE =
  typeof window !== "undefined" ? "/api/ai-trader" : UPSTREAM;
...
export function upstreamConfigured(): boolean {
  return Boolean(process.env.AI_TRADER_API_URL || process.env.NEXT_PUBLIC_AI_TRADER_API_URL);
}
