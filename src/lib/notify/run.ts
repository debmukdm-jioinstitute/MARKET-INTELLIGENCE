...
import { addEvents, claimRun } from "./store";
...
export async function runLiveDetection(minMs = 3 * 60_000): Promise<number> {
  ...
  let added = await addEvents([{ key: "welcome:v1", category: "data", severity: "info", title: "New: the notification centre", body: "This bell lists significant changes across markets, macro data, scanners and AI signals — each with a link to the page that shows it — so you don't have to hunt for what moved.", href: "/Home" }]).catch(() => 0);
  for (const detector of [detectMarket, detectContent]) {
    try {
      added += await addEvents(await detector());
    } catch {
