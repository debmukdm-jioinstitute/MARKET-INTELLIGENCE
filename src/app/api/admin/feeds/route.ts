...
export async function GET() {
  ...
  const db = sql();
  const [datagov, scanner, research, prowessErrors, prowess] = await Promise.all([
  ...
  return NextResponse.json({ counts, series, failures, datagov, scanner, research, prowess, prowessErrors, keys });
}
