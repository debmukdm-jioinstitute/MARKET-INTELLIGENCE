      ...
      {result ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between rounded-lg border border-border bg-card px-4 py-3">
            <div>
              <p className="font-heading text-sm font-semibold">
                {result.name} <span className="text-muted-foreground">({result.symbol})</span>
              </p>
              <p className="text-sm text-muted-foreground">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {result.analysts.map((a) => (
              <div key={a.role} className="rounded-lg border border-border bg-card p-3">
                ...
                <ul className="mt-2 space-y-1 text-sm leading-4 text-muted-foreground">
                  {a.keyPoints.map((p, i) => (
                    <li key={i}>• {p}</li>
                  ))}
                </ul>
                <p className="mt-2 text-sm text-muted-foreground/70">confidence {(a.confidence * 100).toFixed(0)}%</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {result.debate.map((d) => (
              <div key={d.role} className="rounded-lg border border-border bg-card p-3">
                ...
                <p className="mt-1.5 text-sm leading-4 text-muted-foreground">{d.thesis}</p>
                <ul className="mt-2 space-y-1 text-sm leading-4 text-muted-foreground">
                  {d.keyPoints.map((p, i) => (
                    <li key={i}>• {p}</li>
                  ))}
          ...
          <div className="rounded-lg border border-border bg-card p-3">
            <p className="text-sm font-semibold">Trader</p>
            ...
            <p className="mt-1.5 text-sm leading-4 text-muted-foreground">{result.trader.rationale}</p>
          </div>
