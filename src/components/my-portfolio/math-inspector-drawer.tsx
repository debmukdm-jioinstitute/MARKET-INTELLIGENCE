...
export function MathInspectorDrawer({ isOpen, onClose, request }: MathInspectorDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end animate-in fade-in duration-200">
      ...
      <aside
        className={cn(
          "relative z-50 flex h-full flex-col border-l border-border/80 bg-[#090d14] text-foreground shadow-2xl transition-all duration-300 ease-out",
          widthClasses[widthMode]
        )}
        style={{ minWidth: "320px" }}
      >
        {/* Bloomberg Terminal Top Status Bar */}
        <div className="flex items-center justify-between border-b border-border/70 bg-[#0d121c] px-4 py-2.5 font-mono text-xs">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded bg-amber-400/10 px-2 py-0.5 text-[10px] font-bold text-amber-400 uppercase tracking-widest border border-amber-400/20">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              type="button"
              onClick={onClose}
              className="rounded p-1.5 text-muted-foreground hover:bg-rose-500/20 hover:text-rose-300 transition-colors ml-1"
        ...
        <div className="border-b border-border/60 bg-gradient-to-r from-card/80 via-card/50 to-transparent p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="font-mono text-xs font-semibold text-amber-400 tracking-wide uppercase">
                  {request.metricId}
                </span>
            ...
            <div className="rounded-xl border border-amber-400/30 bg-amber-400/5 px-4 py-2 text-right font-mono shadow-inner">
              ...
              <section className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-[11px] font-mono text-muted-foreground hover:text-amber-300 transition-colors"
                  >
                    ...
                    <div className="grid gap-1.5 sm:grid-cols-2">
                      {mathDef.variables.map((v) => (
                        <div key={v.symbol} className="flex items-start gap-2">
                          <Latex math={v.symbol} className="text-amber-400 font-bold" />
                          <div>
                    ...
                    <tbody className="divide-y divide-border/40">
                      {derivation.inputs.map((inp, idx) => (
                        <tr key={idx} className="hover:bg-muted/20 transition-colors">
                          ...
                          <td className="py-2 px-3 text-right font-bold text-amber-200">{inp.value}</td>
              ...
              <section className="space-y-3">
                ...
                <div className="space-y-2.5">
                  {derivation.steps.map((step) => (
                    <div
                      key={step.stepNumber}
                      className="rounded-xl border border-border/80 bg-card/60 p-3.5 space-y-2 font-mono transition-colors hover:border-amber-400/40"
                    >
                      <div className="flex items-center gap-2">
                        <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-amber-400/20 text-[10px] font-bold text-amber-300 border border-amber-400/30">
                          {step.stepNumber}
                        </span>
                ...
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-xs font-mono text-emerald-300 flex items-center gap-2">
        ...
        <div className="flex items-center justify-between border-t border-border/70 bg-[#0d121c] px-5 py-3 font-mono text-xs">
