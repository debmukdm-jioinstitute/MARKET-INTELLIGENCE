...
export function OptionChainResultView({ underlyingKey, label }: { underlyingKey: string; label: string }) {
  ...
  return (
    <div className="space-y-3 p-4 text-sm">
      ...
      <Link href="/markets/derivatives" className="inline-block text-sm text-primary hover:underline">
    ...
    <div className="space-y-3 p-4 text-sm">
      ...
      {q ? (
        <div className="flex items-baseline gap-3">
          ...
          <span className={cn("", q.changePct >= 0 ? "text-emerald-600" : "text-rose-600")}>
